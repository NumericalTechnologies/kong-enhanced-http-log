...
local function send_entries(conf, entries)
  ...
  kong.log.debug(fmt("enhanced-http-log sent data log server, %s:%s HTTP status %d",
...
-- previous queue implementation.  This ensures that enhanced-http-log instances that
...
function _M.execute(conf)
  ...
  kong.log.info(cjson.encode(replace_table_null_values(kong.ctx.plugin.request_headers)))
  ...
  kong.log.info(cjson.encode(replace_table_null_values(kong.ctx.plugin.request_body)))
